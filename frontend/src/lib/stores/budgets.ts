  ...
  try {
    ...
    if (USE_API) {
      try {
        ...
        console.warn(
          "[Budgets] API load failed, falling back to IndexedDB:",
          error
        );
        showToast(
          "Using offline data. Connect to internet for latest data.",
          "warning"
        );
 ...
 * @param options.totalIncome - Optional total income for the budget
 */
export async function createBudgetForCurrentMonth(
  userId: string,
  options?: { name?: string; totalLimit?: number; totalIncome?: number }
): Promise<Budget> {
  ...
  const totalIncome = options?.totalIncome;
  ...
  if (USE_API) {
    try {
      ...
      const newBudget: Budget = {
        ...
        totalIncome: apiBudget.totalIncome,
      ...
      console.log(
        "[Budgets] Created budget via API for month:",
        month,
        "with name:",
        name
      );
  ...
  console.log(
    "[Budgets] Auto-created budget locally for month:",
    month,
    "with name:",
    name
  );
